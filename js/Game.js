class Game {
  ...
  start() {
   ...
   // car2.debug=true
    ...
    this.addSprites(oilGrp, 1, oilCanImg, 0.8);
  }
  play() {
    // background(TrackImg);
    ...
    if(this.collisionWithOIL === true){
      swal(
        {
          title: `Your car just flipped!!`,
          text: "Wanna Try again? If Yes the click down ",
          imageUrl:
  ...
  collisionWithOIL() {
    car1.overlap(oilGrp, function (collector, collected) {
      ...
      flipped.setCollider("rectangle",0,0,100,170)
      collector.remove();
